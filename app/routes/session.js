...
function SessionHandler(db) {
    ...
    this.handleLoginRequest = (req, res, next) => {
        ...
        userDAO.validateLogin(userName, password, (err, user) => {
            // A2 Fix: Use identical error message to prevent username enumeration
            const errorMessage = "Invalid username and/or password";

            if (err) {
                if (err.noSuchUser || err.invalidPassword) {
                    // A2 Fix: Sanitize log output
                    const sanitizedUserName = String(userName || '').replace(/[\r\n\t]/g, '_').substring(0, 50);
                    console.log("Error: failed login attempt for user: ", sanitizedUserName);

                    // A2 Fix: Use identical error message for both cases
                    return res.render("login", {
                        userName: userName,
                        password: "",
            ...
            // A2 Fix: Regenerate session ID after successful login to prevent session fixation
    ...
    const validateSignup = (userName, firstName, lastName, password, verify, email, errors) => {
        ...
        // A2 Fix: Require strong password (min 8 chars, uppercase, lowercase, digit)
